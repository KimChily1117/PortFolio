...
void ULyraCloneAbilitySystemComponent::ProcessAbilityInput(float DeltaTime, bool bGamePaused)
{
	...
	for (const FGameplayAbilitySpecHandle& SpecHandle : InputHeldSpecHandles)
	{
		...
		if (const FGameplayAbilitySpec* AbilitySpec = FindAbilitySpecFromHandle(SpecHandle))
		{
			...
			{
				const ULyraCloneGameplayAbility* HakAbilityCDO = CastChecked<ULyraCloneGameplayAbility>(AbilitySpec->Ability);

				// ActivationPolicy가 WhileInputActive 속성이면 활성화로 등록
				if (HakAbilityCDO->ActivationPolicy == ELyraCloneAbilityActivationPolicy::WhileInputActive)
				{
		...
		if (FGameplayAbilitySpec* AbilitySpec = FindAbilitySpecFromHandle(SpecHandle))
		{
			if (AbilitySpec->Ability)
			{
				...
				else
				{
					...
					if (HakAbilityCDO->ActivationPolicy == ELyraCloneAbilityActivationPolicy::OnInputTriggered)
					{
						AbilitiesToActivate.AddUnique(AbilitySpec->Handle);
					}
				}
