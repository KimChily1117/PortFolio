...
public class NetworkManager
{
    ...
    public bool SendUdpMove(Vector3 position, PlayerState state, MoveDir moveDir)
    {
        return _udpClient.TrySendMove(position, state, moveDir);
    }

    public void SendUdpMoveForTest(Vector3 position, PlayerState state, MoveDir moveDir)
    {
        SendUdpMove(position, state, moveDir);
    }
...
public class UdpGameClient
{
    ...
    bool SendUdpPacket(MsgId msgId, IMessage packet, string logMessage, bool debugOnly = false)
    {
        if (!IsReadyToSend())
            return false;
    ...
    public bool TrySendMove(Vector3 position, PlayerState state, MoveDir moveDir)
    {
        if (!_isRegistered)
        {
            if (GameManager.Network.DebugUdpMovementLog)
                Debug.LogWarning("[UDP] Move skipped until UDP registration succeeds.");
            return false;
        }
        ...
        movePacket.Sequence = _moveSequence;

        return SendUdpPacket(MsgId.CUdpMove, movePacket, $"Move Seq={movePacket.Sequence}, Pos=({position.x},{position.y}), State={state}, MoveDir={moveDir}", GameManager.Network.DebugUdpMovementLog);
    }
}
