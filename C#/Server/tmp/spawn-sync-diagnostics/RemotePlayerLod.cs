...
public class RemotePlayerLod : MonoBehaviour
{
    ...
    void ApplyLevel(LodLevel level, bool force)
    {
        ...
        BaseCharacter remotePlayer = GetComponent<BaseCharacter>();
        BaseCharacter myPlayer = GameManager.ObjectManager != null ? GameManager.ObjectManager.MyPlayer : null;
        float distance = myPlayer != null ? Vector2.Distance(transform.position, myPlayer.transform.position) : -1f;
        Vector3 myPosition = myPlayer != null ? myPlayer.transform.position : Vector3.zero;
        Debug.Log($"[SPAWN_SYNC][LOD] RemoteId={remotePlayer?.Id ?? 0}, RemoteName={name}, Level={level}, Distance={distance:0.00}, RemotePos=({transform.position.x:0.00},{transform.position.y:0.00}), MyPlayerId={myPlayer?.Id ?? 0}, MyPos=({myPosition.x:0.00},{myPosition.y:0.00}), ShowRenderers={showRenderers}, ShowNameLabels={showNameLabels}");
